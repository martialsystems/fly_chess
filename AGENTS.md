...
Public methods note: `docs/method_note.pdf`, pinned on the README. Rebuild with `scripts/make_method_note.py`. Sequel: child-position value (`docs/value_note.md`). Mix-0 occupancy ranker scores legal children and is the player. Mix-1 linear (real vs five shuffle seeds) and a two-layer MLP are probes on hidden rates. Dense self-play catalog, hold out by game. Report vs random-legal and vs mix-0, not Elo. `wiring_helped` is false on the locked table. Mix-1 value line stopped. Graph stays frozen. Truncated-return self-play is closed (`value --stage selfplay` exits). Distill mix-0 is A; do not keep a third mix-0 trainer. Residual toward 2-ply is not promoted. Search is a loop at test time (`value --stage search`): 2-ply/3-ply over the mix-0 eval, 400 ply / resign / count decisive games, same-policy is not 0.50. Mix-1 child distill vs shuffle (`value --stage mix1-distill`) is the only table that can set wiring_helped. Ethology sugar/loom is a separate arm; do not make it the main trainer.
