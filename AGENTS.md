...
Locked numbers live in `logs/ethology_gate.json` and `logs/planes_gate.json` at n=40. README quotes those files. Do not raise n_games on the fixture. `hanging_capture_real_gt_shuffled` is not a win: 774 vs 668 hanging chances. Planes mix-depth table is `logs/planes_mix.json`. Label families are `logs/planes_labels.json`. `logs/planes_head.json` is a one-seed pointer (rank 3.9 vs 1.9). Do not restamp Gate 2 n=40 onto the typed encoder. Do not quote Gate 2 from those tables.
